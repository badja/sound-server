...
ULONG CSocketComm::GetIPAddress( LPCTSTR strHostName )
{
	...
	inet_pton( AF_INET, strHost, &uAddr );
	...
	if ( (INADDR_NONE == uAddr) && (strcmp( strHost, "255.255.255.255" )) )
	{
		...
		if ( getaddrinfo(strHost, NULL, &hints, &result) == 0 )
			uAddr = reinterpret_cast<struct sockaddr_in*>(result->ai_addr)->sin_addr.s_addr;
	};
	...
	if (strAddress != NULL && nSize > 0)
	{
		...
		if (SOCKET_ERROR != gethostname(strHost, sizeof(strHost)))
		{
			...
			if (getaddrinfo(strHost, NULL, &hints, &result) == 0)
			{
				...
				if (inet_ntop(AF_INET, &reinterpret_cast<struct sockaddr_in*>(result->ai_addr)->sin_addr, strHost, HOSTNAME_SIZE) == NULL)
					return false;
...
DWORD CSocketComm::ReadComm(LPBYTE lpBuffer, DWORD dwSize, DWORD dwTimeout)
{
	...
	if ( res > 0)
	{
		...
		{
			SockAddrIn sockAddr; // get peer address
			int nOffset = IsSmartAddressing() ? (int)sockAddr.Size() : 0; // use offset for Smart addressing
...
DWORD CSocketComm::WriteComm(const LPBYTE lpBuffer, DWORD dwCount, DWORD dwTimeout)
{
	...
	if ( res > 0)
	{
		...
		if (IsBroadcast() || IsSmartAddressing())
		{
			...
			if (IsSmartAddressing())
			{
				...
				if (sockAddr.sin_addr.s_addr != htonl(INADDR_BROADCAST))
				{
					res = sendto( s, (LPCSTR)&lpBuffer[nOffset], dwCount-nOffset, 0,
						(LPSOCKADDR)sockAddr, (int)sockAddr.Size());
			...
			{
				res = sendto( s, (LPCSTR)&lpBuffer[nOffset], dwCount-nOffset, 0, (LPSOCKADDR)(*iter), (int)iter->Size());
				if (res < 0)
				{
